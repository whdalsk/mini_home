...
<%@ taglib uri="http://java.sun.com/jsp/jstl/fmt" prefix="fmt" %>l>
<html lang="en">
<head>
    <meta charset="UTF-8">
    ...
    <table width="800"  border=1   cellspacing="0" class="myTable">
	  	 ...
	  	 <c:choose>
		    <c:when test="${startpage>10}">
		        <a href="diary_R2.do?idx=${startpage - 10 }">[이전]</a>
		    </c:when>
		    <c:otherwise>
		        <!-- 아무것도 하지 않음 -->
		    </c:otherwise>
	  	 ...
	  	 <c:forEach var="i" begin="${startpage}" end="${endpage}" step="1">
		    ...
		    <c:when test="${endpage < pagecount}">
		        <a href="diary_R2.do?idx=${startpage + 10}">[다음]</a>
		    </c:when>
		    <c:otherwise>
		        <!-- 아무것도 하지 않음 -->
		    </c:otherwise>
